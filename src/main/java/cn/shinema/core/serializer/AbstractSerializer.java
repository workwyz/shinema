...
package cn.shinema.core.serializer;
...
public class AbstractSerializer {
	...
	protected AbstractSerializer() {
		this.gson = new GsonBuilder().registerTypeAdapter(Date.class, new StringDateSerializer())
				.registerTypeAdapter(Date.class, new StringDateDeserializer()).serializeNulls().create();
	}
	...
	private class StringDateSerializer implements JsonSerializer<Date> {
		public JsonElement serialize(Date source, Type typeOfSource, JsonSerializationContext context) {
			return new JsonPrimitive(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(source.getTime()));
		}
	}

	private class StringDateDeserializer implements JsonDeserializer<Date> {
		public Date deserialize(JsonElement json, Type typeOfTarget, JsonDeserializationContext context) {
